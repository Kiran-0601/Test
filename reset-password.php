...
if (isset($_POST['submit'])) {
  ...
  if ($result->num_rows > 0) {
    ...
    if (strtotime($addedTime) > strtotime($currentDateTime)) {
      ...
      // Invalid password, show an error message
      echo "<div id='alertMessage' class='alert alert-danger message-container fade show position-fixed top-0 end-0' role='alert'>
      Sorry !! This link is expired &nbsp;&nbsp;  <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
      </div>";
    ...
    // Invalid password, show an error message
    echo "<div id='alertMessage' class='alert alert-danger message-container fade show position-fixed top-0 end-0' role='alert'>
    This link is not valid. Click to Forgot password . &nbsp;&nbsp;  <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
    </div>";
  }
}
?>
